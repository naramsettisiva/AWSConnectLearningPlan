...
function buildTrack(TRACK){
  ...
  TRACK.days.forEach(d=>{
    ...
    strip.innerHTML = `
      ...
      <div class="strip-body">
        ...
        ${d.links && d.links.length ? `<div class="links">${d.links.map(l=>`<a href="${l.url}" target="_blank" rel="noopener">${l.label} ↗</a>`).join('')}</div>` : ''}
        ${d.setup && d.setup.length ? `<div class="setup"><div class="setup-title">Lab setup — self-guided</div><ol class="setup-steps">${d.setup.map(s=>`<li>${s}</li>`).join('')}</ol></div>` : ''}
        ${d.quiz && d.quiz.length ? renderQuiz(quizId, d.quiz, (c,t)=>{ scores[d.n] = {correct:c, total:t, ts:Date.now()}; saveJSON(SCORES_KEY, scores); }) : ''}
      </div>
    `;
  ...
  TRACK.days.forEach(d=>{
    if(d.quiz && d.quiz.length) wireQuiz('day'+d.n, d.quiz, ()=>{});
  });
